...
describe('updateHealthState', () => {
	it('keeps a healthy connection quiet while showing the Nexus update age', () => {
		expect(updateHealthState(true, 0, 2 * minute + 14 * second)).toEqual({
			label: 'Nexus updated 2 min 14 sec ago',
			hasProblem: false,
		});
	});
	...
	it('calls out outdated Nexus data and server disconnections', () => {
		expect(updateHealthState(true, 0, 5 * minute)).toEqual({
			label: 'Nexus outdated · updated 5 min 0 sec ago',
			hasProblem: true,
		});
		expect(updateHealthState(false, 0, 6 * minute)).toEqual({
			label: 'Server disconnected · Nexus outdated · updated 6 min 0 sec ago',
			hasProblem: true,
		});
